...
void ObjectSerializer::Serialize(const ISerializable* pObject, string objFilename)
{
    ...
    auto objLayout = pObject->GetObjectLayout();
...
{ 
    ...
    if(isPtr)
    {
        ...
        auto        objLayout = pObj->GetObjectLayout();
        unsigned*   addr32 = reinterpret_cast<unsigned*>(objLayout.RootAddress());
        ...
        if(!isNull)
        {
            ...
            PerformLateBinding(objLayout, pType);
...
int ObjectSerializer::SerializeContainerVector(ISerializable* pObj, TypeNode* pType, fstream& pen)
{
    ...
    auto        objLayout = pObj->GetObjectLayout();
    Container*  container = reinterpret_cast<Container*>(objLayout.RootAddress());
...
int ObjectSerializer::DeserializeType(char* pMem, TypeNode* pType, fstream& eye)
{
    ...
    case DTYPE_Vector:
        return DeserializeContainerVector(pMem, pType, eye);

    case DTYPE_Map:
        return DeserializeContainerMap(pMem, pType, eye);

    case DTYPE_Set:
        return DeserializeContainerSet(pMem, pType, eye);

    case DTYPE_Pair:
        return DeserializePair(pMem, pType, eye);
    ...
    case DTYPE_UserDefined:
        {
            ...
            _ASSERTE(m_typeTable.find(fullName) != m_typeTable.end());
            return DeserializeUserDefinedType(pMem, m_typeTable[fullName].TypeGraph, pType->Indirection, eye);
        }
...
//----------------------------------------------------------------------------------------------
int ObjectSerializer::DeserializeContainerVector(char* pMem, TypeNode* pType, fstream& eye)
{
    // Pointer to vector is not supported
    _ASSERTE(pType->Indirection == false);

    Container* container = reinterpret_cast<Container*>(pMem);
    int count;
...
//----------------------------------------------------------------------------------------------
int ObjectSerializer::DeserializeContainerMap(char* pMem, TypeNode* pType, fstream& eye)
{
    // Pointer to map is not supported
    _ASSERTE(pType->Indirection == false);
...
//----------------------------------------------------------------------------------------------
int ObjectSerializer::DeserializeContainerSet(char* pMem, TypeNode* pType, fstream& eye)
{
    // Pointer to set is not supported
    _ASSERTE(pType->Indirection == false);
